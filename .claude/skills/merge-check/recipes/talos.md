...
- `KubeCoreDNSConfig`, `KubeNodeConfig`, `KubeTalosAPIAccessConfig` are `not registered`, so `cluster.coreDNS`, `cluster.allowSchedulingOnControlPlanes` and `machine.features.kubernetesTalosAPIAccess` stay v1alpha1 until talhelper ships >= 1.14.0 machinery. Re-check by putting the kind in a controller patch and rendering: the failure is `error decoding document v1alpha1/<Kind>/ (line 1): "<Kind>" "v1alpha1": not registered`. Still all three on 2026-09-13; v3.1.17 is the newest release and talhelper `master` is also on `v1.14.0-alpha.2`, with budimanjojo/talhelper#1708 open at rc.2.
